...
public class Principal extends JFrame {
	...
	private ThreadCharacterController[] threadCharacters = new ThreadCharacterController[7];
	...
	public Principal() {
		...
		SelectCharacterButtonController selectCharacterButtonController = new 
				SelectCharacterButtonController(btnStartRace, this);
		btnSelectCharacters.addActionListener(selectCharacterButtonController);
		
		StartRaceButtonController startRaceButtonController = new 
				StartRaceButtonController(btnStartRace, btnSelectCharacters, threadCharactersInRace, 
						 contentPane);
		...
		for (int i = 0; i < lblCharacters.length; i++) {
			...
			threadCharacters[i] = new ThreadCharacterController(charactersNames[i], lblCharacters[i], btnStartRace, btnSelectCharacters);
		
			threadCharacters[i].createAnimation("idle", 1, new Rectangle(96, 0 + (i * 32), 32, 32));
			threadCharacters[i].createAnimation("lose", 1, new Rectangle(544, 0 + (i * 32), 32, 32));
			threadCharacters[i].createAnimation("run", charactersTotalRunFrames[i], new Rectangle(160, 0 + (i * 32), 32, 32));
			threadCharacters[i].createAnimation("win", charactersTotalWinFrames[i], new Rectangle(384, 0 + (i * 32), 32, 32));
			threadCharacters[i].updateLabel();
		}
	...
	public void setCharactersInRace(int[] charactersInRaceId) {
		for (int i = 0; i < charactersInRaceId.length; i++) {
			threadCharactersInRace[i] = threadCharacters[charactersInRaceId[i]];
		}
	}
